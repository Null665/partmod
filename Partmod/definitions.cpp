...
"Impossible to create a primary parttion on extended partition",
...
if(_offset_rel>_gh.length*bps)
    return ERR_INVALID_SEEK;

_gh.hDisk->Seek(_offset_rel*bps);

if(_gh.hDisk->Write(_buff,_size))
    return ERR_WRITE;
...
/*
int DllBootInfo(GEN_HANDLE _gh)
{
DLL_BOOT_INFO fp=0;
DllCall dll;
switch(_gh.fsid)
  {
    ...
    case FS_NTFS:
      dll.Open("fs\\ntfs.dll");
      break;
    default: MessageBox(0,"File system not supported","Error",48); return -1; break;

  }

fp=dll.GetProc<DLL_BOOT_INFO>("BootInfo");
...
int DllResizePartition(GEN_HANDLE _gh,uint64_t _sectors,bool _increase)
{
...
if(hi==NULL)
  {
    MessageBox(0,"Failed to load dll","Error",16);
    return -2;
  }

fp=(DLL_RESIZE)GetProcAddress(hi,"Resize");


if(fp==NULL)
  {
    MessageBox(0,"Could not access a dll function ","Error",16);
    FreeLibrary(hi);
    return -3;
  }

int ret=fp(_gh,_sectors,_increase);
FreeLibrary(hi);
return ret;
...
fp=(DLL_FORMAT)GetProcAddress(hi,"Format");
...
fp=(DLL_CALC_FREE)GetProcAddress(hi,"CalcFree");
...
int DllFsCheck(GEN_HANDLE _gh,string &_err_log)
{
HINSTANCE hi;
DLL_FS_CHECK fp;
switch(_gh.fsid)
...
fp=(DLL_FS_CHECK)GetProcAddress(hi,"Check");
